...
{ 
  ...
  For ($z=0 ; $z -le $max ; $z++)
  {
    #Start-Sleep -Milliseconds $rand.Next(7,137)
    $los[$z]=Get-Random -Minimum $min -Maximum $max
    IF ( $los[$z] -eq $los[$z-1] ) { $los[$z]=Get-Random -Minimum $min -Maximum $max }
    IF ( $z -gt 1 -and $los[$z] -eq $los[$z-2] ) { $los[$z]=Get-Random -Minimum $min -Maximum $max }
    IF ( $z -gt 2 -and $los[$z] -eq $los[$z-3] ) { $los[$z]=Get-Random -Minimum $min -Maximum $max }
  }
  $los= $los |Get-Random
  return $los
} 
...
{  
    ...
    For($x = 1 ; $x -le $ilosc ; $x++ )                                                       # 
        {                                                                                     # 
            ...
            IF ($ktorafunkcja -le 4) {                                                        # |Generowanie liczb losowych
                                                          $liczba[$x] = LosujLiczbe $min $max # |dla poszczególnych składników 
